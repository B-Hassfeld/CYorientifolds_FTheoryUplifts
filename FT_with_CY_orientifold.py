...
class CY_orientifold():
    ...
    def __init__(self, fan_polytope_or_points=None, xi=None, resolve_A1_singularities=True, triangulate_points=False, construct_nef_decomposition=True):
        ...
        match fan_polytope_or_points:
            case Polytope():
                ...
                if len(ind) > 0:
                    toric_fan = VectorConfiguration(np.delete(fan_polytope_or_points.points(), ind[0], axis=0))
                else:
                    toric_fan = VectorConfiguration(fan_polytope_or_points.points())
            ...
            case np.ndarray():
                if xi is None:
                    raise ValueError("xi has to be defined")
                toric_fan = VectorConfiguration(fan_polytope_or_points)
            case list():
                if xi is None:
                    raise ValueError("xi has to be defined")
                toric_fan = VectorConfiguration(np.array(fan_polytope_or_points))
        ...
        if self.__regular:
            if construct_nef_decomposition:
                ...
                if len(KB_multiplier.points()) > 0:
                    NP_orbifold = UF.Newton_Polytope(orbifold_fan.vectors(), orbifold_line_bundle)
                    self.__Newton_Polytope = NP_orbifold
                    if NP_orbifold.minkowski_sum(KB_multiplier).dim() == orbifold_fan.dim:
    ...
    def dim(self):
        ...
        return self.orbifold_toric_fan().dim
    ...
    def ambient_dim(self):
        ...
        return self.orbifold_toric_fan().ambient_dim
    ...
    def triangulate(self):
        ...
        if not self.triangulated():
            if self.yields_nef_decomposition():
                ...
                self.__orbifold_toric_fan = UF.refine_fan(prelim_fan, self.orbifold_toric_fan().vectors())
            self.__triangulated = True
...
class F_Theory_Uplift():
    ...
    def points_singular_uplift(self, labels=None):
        ...
        return self.singular_uplift_toric_fan().vectors(labels)
    ...
    def M_conv_toric_fan(self):
        if type(self.__M_conv_toric_fan) == type(None):
    ...
    def points_smooth_uplift(self, labels=None):
        ...
        return self.smooth_uplift_toric_fan().vectors(labels)
